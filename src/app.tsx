...
import { resolveLocalTrackUri } from "./utils/SpotifyLocalTrackResolver.ts";
...
async function main() {
  ...
  const Hometinue = async () => {
    ...
    {
      let lastTimeout: any;
      let lastResolvedUri: string | undefined;
      Global.Event.listen("lyrics:apply", () => {
        ...
        lastTimeout = setTimeout(async () => {
          ...
          const resolvedUri = await resolveLocalTrackUri(liveUri);
          const currentSongLyrics = $currentLyricsData.get();
          const noLyricsSentinel = `NO_LYRICS:${resolvedUri}`;

          if (!currentSongLyrics || currentSongLyrics === noLyricsSentinel) {
            return;
          }

          if (lastResolvedUri === resolvedUri) {
            return;
          }

          try {
            const parsedLyrics = JSON.parse(currentSongLyrics);
            if (parsedLyrics?.uri !== resolvedUri) {
